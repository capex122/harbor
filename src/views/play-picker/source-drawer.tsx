...
export function SourceDrawer({
  ...
  useEffect(() => {
    ...
    if (!open || !root || visibleCount >= shown.length) return;

    const onScroll = () => {
      const remaining = root.scrollHeight - root.scrollTop - root.clientHeight;
      if (remaining > 320) return;
      setVisibleCount((count) =>
        count === visibleCount ? Math.min(count + STREAM_PAGE_SIZE, shown.length) : count,
      );
    };
    root.addEventListener("scroll", onScroll, { passive: true });
    return () => root.removeEventListener("scroll", onScroll);
  }, [open, scrollRef, shown.length, visibleCount]);
  const visibleStreams = shown.slice(0, visibleCount);
  return (
