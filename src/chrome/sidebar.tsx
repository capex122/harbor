  ...
  return (
    <>
      <aside
        ...
        data-harbor-sidebar
        className={`relative z-[60] hidden w-[72px] shrink-0 flex-col border-e border-edge-soft bg-canvas transition-[opacity,transform,width] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${mobileOnly ? "" : "lg:flex"} ${
          collapsed ? "" : "lg:w-60"
        } ${
          chromeHidden
...
const MOBILE_NAV: Array<{ id: NavItemId; label?: string; menu?: NavItemId[] }> = [
  { id: "discover", menu: ["movies", "shows", "anime"] },
          ...
          <div key={nav.id} className="relative flex min-w-0 flex-1 justify-center">
            ...
            >
              <span className={isHome ? "flex h-14 w-14 items-center justify-center rounded-full bg-ink text-canvas shadow-[0_8px_24px_rgba(0,0,0,0.38)]" : "flex h-8 items-center justify-center"}>
                {displayed.render(active)}
              </span>
              {!isHome && <span className="max-w-full truncate">{t(displayedLabel)}</span>}
            </button>
          </div>
        );
      })}
